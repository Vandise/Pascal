...
SYMTAB_NODE_PTR formal_parm_list(int * countp, int * total_sizep) {
  ...
  */
  while ((token == IDENTIFIER) || (token == VAR)) {
    ...
    */
    for (parm_idp = first_idp; parm_idp != NULL; parm_idp = parm_idp->next) parm_idp->typep = parm_tp;
  ...
  */
  ...
  for (parm_idp = parm_listp; parm_idp != NULL; parm_idp = parm_idp->next) {
    ...
    parm_offset += parm_idp->defn.key == VALPARM_DEFN ? parm_idp->typep->size : sizeof(char * );
    if (parm_offset & 1) ++parm_offset; /* round up to even */
  }
  reverse_list( & parm_listp);

  if_token_get_else_error(RPAREN, MISSING_RPAREN); * countp = parm_count; * total_sizep = parm_offset - PARAMETERS_STACK_FRAME_OFFSET;
