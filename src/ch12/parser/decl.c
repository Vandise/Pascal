...
void do_const(SYMTAB_NODE_PTR const_idp) {
  ...
  if (token == NUMBER) {
    if (literal.type == INTEGER_LIT) {
      const_idp->defn.info.constant.value.integer =
        sign == PLUS ? literal.value.integer : -literal.value.integer;
      const_idp->typep = integer_typep;
    } else {
      const_idp->defn.info.constant.value.real =
        sign == PLUS ? literal.value.real : -literal.value.real;
  ...
  */
    ...
    if (idp == NULL)
      ...
      const_idp->defn.info.constant.value.integer =
        sign == PLUS ? idp->defn.info.constant.value.integer : -idp->defn.info.constant.value.integer;
      const_idp->typep = integer_typep;
    } else if (idp->typep == real_typep) {
      const_idp->defn.info.constant.value.real =
        sign == PLUS ? idp->defn.info.constant.value.real : -idp->defn.info.constant.value.real;
      const_idp->typep = real_typep;
...
void get_subrange_limit(SYMTAB_NODE_PTR minmax_idp, int *minmaxp, TYPE_STRUCT_PTR *typepp) {
  ...
  */
  if (token == NUMBER) {
    if (literal.type == INTEGER_LIT) { * typepp = integer_typep; * minmaxp = (sign == PLUS) ? literal.value.integer : -literal.value.integer;
  ...
  */
    ...
    else if (idp->defn.key == CONST_DEFN) { * typepp = idp->typep;
      if (idp->typep == char_typep) {
        if (saw_sign) error(INVALID_CONSTANT); * minmaxp = idp->defn.info.constant.value.character;
      } else if (idp->typep == integer_typep) { * minmaxp = idp->defn.info.constant.value.integer;
        if (sign == MINUS) * minmaxp = -( * minmaxp);
      } else /* enumeration constant */ {
        if (saw_sign) error(INVALID_CONSTANT); * minmaxp = idp->defn.info.constant.value.integer;
      }
  ...
  */
    ...
    if (saw_sign) error(INVALID_CONSTANT); * typepp = char_typep; * minmaxp = literal.value.string[0];
...
void var_or_field_declarations(SYMTAB_NODE_PTR rtn_idp, TYPE_STRUCT_PTR record_tp, int offset) {
   SYMTAB_NODE_PTR idp, first_idp, last_idp;
   /* variable or
						     field ids */
